...
class ClientCell extends PositionedObject {
  ...
  initGameObjects() {
    ...
    this.objects = cellCfg.map((layer, layerId) =>
      layer.map((objCfg) => {
        let ObjectClass;
        ObjectClass = objCfg.class ? objectClasses[objCfg.class] : ClientGameObject;
  ...
  addGameObject(objToAdd) {
    const { objects } = this;
    if (objToAdd.layerId === undefined) objToAdd.layerId = objects.length;
    if (!objects[objToAdd.layerId]) objects[objToAdd.layerId] = [];
